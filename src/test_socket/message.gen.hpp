...
public:
    enum {rtt = 0};
    bool operator == (message_join const& other) const
    {
        return true;
    }
    bool operator != (message_join const& other) const
...
}   // end of namespace meshpp_message
namespace std
{
//  provide a simple hash, required by std::unordered_map
template <>
struct hash<meshpp_message::message_join>
{
    size_t operator()(meshpp_message::message_join const& value) const noexcept
    {
        std::hash<string> hasher;
        return hasher(meshpp_message::detail::saver(value));
    }
};
...
public:
    enum {rtt = 1};
    bool operator == (message_drop const& other) const
    {
        return true;
    }
    bool operator != (message_drop const& other) const
...
//  provide a simple hash, required by std::unordered_map
template <>
struct hash<meshpp_message::message_drop>
{
    size_t operator()(meshpp_message::message_drop const& value) const noexcept
    {
        std::hash<string> hasher;
        return hasher(meshpp_message::detail::saver(value));
    }
...
public:
    enum {rtt = 2};
    bool operator == (message_error const& other) const
    {
        return true;
    }
    bool operator != (message_error const& other) const
...
//  provide a simple hash, required by std::unordered_map
template <>
struct hash<meshpp_message::message_error>
{
    size_t operator()(meshpp_message::message_error const& value) const noexcept
    {
        std::hash<string> hasher;
        return hasher(meshpp_message::detail::saver(value));
    }
...
namespace detail
{
std::string saver(Sum const& self);
}
class Sum
{
public:
    ...
    {}
    template <typename T>
    explicit Sum(T const& other)
    {
        ::beltpp::assign(first, other.first);
        ::beltpp::assign(second, other.second);
    }
    ...
    {
        ::beltpp::assign(first, std::move(other.first));
        ::beltpp::assign(second, std::move(other.second));
        return *this;
    }
    template <typename T>
    Sum& operator = (T const& other)
    {
        ::beltpp::assign(first, other.first);
        ::beltpp::assign(second, other.second);
        return *this;
    ...
    bool operator < (Sum const& other) const
    {
        if (false == detail::less(first, other.first))
            return false;
        if (false == detail::less(second, other.second))
            return false;
        return true;
    }
    bool operator > (Sum const& other) const
    ...
    static std::vector<char> saver(void* p)
    {
        Sum* pmc = static_cast<Sum*>(p);
        std::vector<char> result;
        std::string str_value = detail::saver(*pmc);
        std::copy(str_value.begin(), str_value.end(), std::back_inserter(result));
        return result;
    }
    static std::string string_saver(Sum const& ob)
    {
        return detail::saver(ob);
    }
    static void string_loader(Sum& ob, std::string const& encoded)
    {
        if (false == detail::loader(ob, encoded, nullptr))
            throw std::runtime_error("cannot parse Sum data");
    }
};  // end of class
}   // end of namespace meshpp_message
namespace beltpp
{
template <typename T>
void assign(meshpp_message::Sum& self, T const& other)
{
    ::beltpp::assign(self.first, other.first);
    ::beltpp::assign(self.second, other.second);
}
template <typename T, typename = typename std::enable_if<!std::is_same<T, meshpp_message::Sum>::value, void>::type>
void assign(T& self, meshpp_message::Sum const& other)
{
    ::beltpp::assign(self.first, other.first);
    ::beltpp::assign(self.second, other.second);
}
template <typename T>
void assign(T& self, meshpp_message::Sum&& other)
{
    ::beltpp::assign(self.first, std::move(other.first));
    ::beltpp::assign(self.second, std::move(other.second));
}
                  ...
                  ::beltpp::message_loader_utility const& utl)
{
    ...
    else
    {
        if (code)
        {
            auto it_find = members.find("\"first\"");
            ...
            else
            {
                ...
                assert(item);
                code = analyze_json(msgcode.first, item, utl);
            }
        }
        if (code)
        {
            auto it_find = members.find("\"second\"");
            if (it_find == members.end())
                code = false;
            else
            {
                beltpp::json::expression_tree* item = it_find->second;
                assert(item);
                code = analyze_json(msgcode.second, item, utl);
            }
        }
    }
    return code;
...
std::string saver(Sum const& self)
{
    std::string result;
    result += "{";
    result += "\"rtt\":" + saver(Sum::rtt);
    result += ",\"first\":" + saver(self.first);
    result += ",\"second\":" + saver(self.second);
    result += "}";
    return result;
}
...
//  provide a simple hash, required by std::unordered_map
template <>
struct hash<meshpp_message::Sum>
{
    size_t operator()(meshpp_message::Sum const& value) const noexcept
    {
        std::hash<string> hasher;
        return hasher(meshpp_message::detail::saver(value));
    }
...
namespace detail
{
std::string saver(SumResult const& self);
}
class SumResult
{
public:
    ...
    SumResult()
      : value()
    {}
    SumResult(SumResult const& other)
      : value(other.value)
    {}
    template <typename T>
    explicit SumResult(T const& other)
    {
        ::beltpp::assign(value, other.value);
    }
    SumResult& operator = (SumResult const& other)
    {
        ::beltpp::assign(value, other.value);
        return *this;
    }
    SumResult& operator = (SumResult&& other)
    {
        ::beltpp::assign(value, std::move(other.value));
        return *this;
    }
    template <typename T>
    SumResult& operator = (T const& other)
    {
        ::beltpp::assign(value, other.value);
        return *this;
    }
    bool operator == (SumResult const& other) const
    {
    ...
    bool operator < (SumResult const& other) const
    {
        if (false == detail::less(value, other.value))
            return false;
        return true;
    }
    bool operator > (SumResult const& other) const
    ...
    static std::vector<char> saver(void* p)
    {
        SumResult* pmc = static_cast<SumResult*>(p);
        std::vector<char> result;
        std::string str_value = detail::saver(*pmc);
        std::copy(str_value.begin(), str_value.end(), std::back_inserter(result));
        return result;
    }
    static std::string string_saver(SumResult const& ob)
    {
        return detail::saver(ob);
    }
    static void string_loader(SumResult& ob, std::string const& encoded)
    {
        if (false == detail::loader(ob, encoded, nullptr))
            throw std::runtime_error("cannot parse SumResult data");
    }
};  // end of class
}   // end of namespace meshpp_message
namespace beltpp
{
template <typename T>
void assign(meshpp_message::SumResult& self, T const& other)
{
    ::beltpp::assign(self.value, other.value);
}
template <typename T, typename = typename std::enable_if<!std::is_same<T, meshpp_message::SumResult>::value, void>::type>
void assign(T& self, meshpp_message::SumResult const& other)
{
    ::beltpp::assign(self.value, other.value);
}
template <typename T>
void assign(T& self, meshpp_message::SumResult&& other)
{
    ::beltpp::assign(self.value, std::move(other.value));
}
}   // end of namespace beltpp
namespace meshpp_message
{
...
{
inline
bool analyze_json(SumResult& msgcode,
                  beltpp::json::expression_tree* pexp,
                  ::beltpp::message_loader_utility const& utl)
{
    bool code = true;
    ...
    else
    {
        if (code)
        {
            auto it_find = members.find("\"value\"");
            if (it_find == members.end())
                code = false;
            else
            {
                beltpp::json::expression_tree* item = it_find->second;
                assert(item);
                code = analyze_json(msgcode.value, item, utl);
            }
        }
    }
    return code;
...
std::string saver(SumResult const& self)
{
    std::string result;
    result += "{";
    result += "\"rtt\":" + saver(SumResult::rtt);
    result += ",\"value\":" + saver(self.value);
    result += "}";
    return result;
}
}   //  end of namespace detail
...
//  provide a simple hash, required by std::unordered_map
template <>
struct hash<meshpp_message::SumResult>
{
    size_t operator()(meshpp_message::SumResult const& value) const noexcept
    {
        std::hash<string> hasher;
        return hasher(meshpp_message::detail::saver(value));
    }
...
std::vector<typename storage<T>::storage_item> const storage<T>::s_arr_fptr =
{
    ...
    {
        &Sum::saver,
        &storage<>::analyze_json_template<Sum>,
        (storage<>::fptr_new_void_unique_ptr)&::beltpp::new_void_unique_ptr<Sum>,
        (storage<>::fptr_new_void_unique_ptr_copy)&::beltpp::new_void_unique_ptr_copy<Sum>
    },
    {
        &SumResult::saver,
        &storage<>::analyze_json_template<SumResult>,
        (storage<>::fptr_new_void_unique_ptr)&::beltpp::new_void_unique_ptr<SumResult>,
        (storage<>::fptr_new_void_unique_ptr_copy)&::beltpp::new_void_unique_ptr_copy<SumResult>
    }
};
...
{
    if (meshpp_message::detail::storage<>::s_arr_fptr.size() <= other.type())
    {
        assert(false);
        std::terminate();
    }

    auto const& item = meshpp_message::detail::storage<>::s_arr_fptr[other.type()];
